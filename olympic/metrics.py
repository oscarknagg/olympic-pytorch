...
def top_k_accuracy(y: torch.Tensor, y_pred: torch.Tensor, top_k: int = 5) -> float:
...
def mean_absolute_error(y: torch.Tensor, y_pred: torch.Tensor) -> float:
    """Calculates mean absolute error.

    # Arguments:
        y: Ground truth values. Must have shape [batch_size, *] where * is any additional
            number of dimensions
        y_pred: Predicted vales of shape [batch_size,*] where * is any additional number
            of dimensions. Must have the same size as y
    """
    return F.l1_loss(y_pred, y, reduction='elementwise_mean').item()


def mean_squared_error(y: torch.Tensor, y_pred: torch.Tensor) -> float:
    ...
    # Arguments:
        ...
        y_pred: Predicted vales of shape [batch_size,*] where * is any additional number
            of dimensions. Must have the same size as y
    """
    return F.mse_loss(y_pred, y, reduction='elementwise_mean').item()
